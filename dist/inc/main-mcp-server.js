...
function addTestTool(mainMcpServer, config) {
    mainMcpServer.tool(getToolName(config, 'echo'), "Just a simple echo tool, will be removed in the future", {
        input: z.string()
    }, async ({ input }) => ({
        content: [{
                type: "text",
...
async function startMainServerOld({ prefix, wrappedServerArgs }) {
    const mainMcpServer = new McpServer({
        name: prefix,
        version: "1.0.0"
    });
    mainMcpServer.tool(prefix + `_` + `echo`, "Just a simple echo tool, will be removed in the future", {
        input: z.string()
    }, async ({ input }) => ({
        content: [{
                type: "text",
